...
public class Grid : MonoBehaviour, IPointerExitHandler
{
    ...
    private Cell[,] _cells;
    private Cell _selectedCell;//ячейка, на которую навели курсор
    ...
    {
        if (_availableMoves.Contains(_selectedCell))
        {
    ...
    public void OnUpdate()
    {
        ...
        _selectedCell = GetCell();
        if (_availableMoves.Contains(_selectedCell))
        {            
            _selectedQuad.transform.position = _selectedCell.position;
        }
        else
        {            
            HideSelectedQuad();
    ...
    void CreateCellsArray()
    {
        ...
        for (int x = 0; x < _rowCount; x++)
        {
            for (int y = 0; y < _colCount; y++)
            {
                ...
                cell.position = new Vector3(x * _cellSize - _offset, _yHeight, y * _cellSize - _offset);
                _cells[x, y] = cell;
            }
        }
    }
    ...
    public Vector2Int GetCellIndexes(float xPos, float zPos)
    {
        ...
        inds.x = Mathf.Clamp( Mathf.RoundToInt(xPos + _offset / _cellSize), 0, _rowCount - 1);
        inds.y = Mathf.Clamp( Mathf.RoundToInt(zPos + _offset / _cellSize), 0, _colCount - 1);
        return inds;
    }
    ...
    public void CheckMoveVariations(Move processor, Cell curretnCell)
    {
